                     ...
                     sidebarLayout(
                       sidebarPanel(
                         ...
                         fluidRow(
                           column(6,
                                  ...
                                  # "blckmdlngApproach"
                                  radioButtons(inputId = "blckmdlngApproach",
                       ...
                       mainPanel(
                         sidebarLayout(
                             ...
                             # 5.1.2(C) Pre-specified block types####
                             conditionalPanel(
                               ...
                               sidebarPanel(
                                 ...
                                 conditionalPanel(
                                   ...
                                   fluidRow(
                                     column(width = 6,
                                            numericInput(inputId = "blckmdlngPreSpecifiedLengthDimension1",
                                                         label = 'Max number of block types for each block',
                                                         value = 1,
                                                         min = 1,
                                                         step = 1),
                                            
                                            numericInput(inputId = "blckmdlngPreSpecifiedLengthDimension2",
                                                         label = 'Number of relations*',
                                                         value = 1,
                                                         min = 1,
                                                         step = 1),
                                            withTags(i('*(1 for single-relational networks)')),
                                     ),
                                     column(width = 6,
                                            numericInput(inputId = "blckmdlngPreSpecifiedLengthDimension3",
                                                         label = 'Cluster rows\' size',
                                                         value = 1,
                                                         min = 1,
                                                         step = 1),
                                            
                                            numericInput(inputId = "blckmdlngPreSpecifiedLengthDimension4",
                                                         label = 'Cluster columns\' size',
                                                         value = 1,
                                                         min = 1,
                                                         step = 1)
                                     ), # column
                                   ...
                                   # 5.1.2(D) Hot table ####
                                   conditionalPanel(
                                     condition = 'input.blckmdlngPreSpecifiedLengthDimension1==1&&input.blckmdlngPreSpecifiedLengthDimension2==1',
                                     rHandsontableOutput(outputId = 'PreSpecifiedBlocktypesHOT',
                                                         width = '100%',height='100%'),
                                   ),
                                   # Hot table not available 
                                   conditionalPanel(
                                     condition = 'input.blckmdlngPreSpecifiedLengthDimension1!=1||input.blckmdlngPreSpecifiedLengthDimension2!=1',
                                     withTags(i("Liva table input not supported, please upload a file instead")),
                                   ),
                                 ),
                                 
                                 hr(),
                                 
                                 # File upload for pre-specified block-types' array
                                 fluidRow(
                                   column(width = 6,
                                          # 5.1.3(E) Use a sample ####
                                          # "Sample" 
                                          ...
                                          conditionalPanel(
                                            ...
                                            # 5.1.3(F) Type of uploaded array ####
                                            # "ArrayInput"
                                            ...
                                            # 5.1.3(G) Uplad array as RDS file ####
                                            conditionalPanel(
                                            ...
                                            # 5.1.3(H) Uplad array as RData file ####
                                            conditionalPanel(
                         ...
                         fluidRow(
                           column(3,# 5.1.11 Start blockmodeling ####
                                  withTags(
                                    h4(
                           ...
                           column(4,offset = 1,
                                  # 5.1.12 Download vector partitions ####
                                  withTags(
                                    h5(
...
server <- function(input, output) {
  ...
  output$NetworkPlot<-renderPlot({
    ...
    if(input$PlotSys==1){
      ...
      # 7.2.2 With or without partitions?
      if(input$PlotSelector==2){
        VertexCol<-"cluster"
      } else {
        VertexCol<-2
    ...
    if(input$blckmdlngPrespecifiedYN){
      if(input$SampleArray){
        ...
        if(condition){
          ...
          # Reading data from HOT table
          BlockTypes<-array(NA,dim = c(1,1,
                                       input$blckmdlngPreSpecifiedLengthDimension3,
                                       input$blckmdlngPreSpecifiedLengthDimension4))
          BlockTypes[1,1,,]<-GetPresepcifiedBlockTypes()
        } else {
  ...
  output$PreSpecifiedBlocktypesHOT<-
    renderRHandsontable({
      hot_tbl<-as.data.frame(
      matrix(NA,nrow = input$blckmdlngPreSpecifiedLengthDimension3,
             ncol = input$blckmdlngPreSpecifiedLengthDimension4)
      )
    colnames(hot_tbl)<-1:ncol(hot_tbl)
    
    hot<-rhandsontable::rhandsontable(data = hot_tbl,useTypes = T)%>%
      hot_cols(type= 'dropdown',strict = T, copyable = F,default="type",
               source = c("nul","com","rdo", "cdo","reg","rre",
                          "cre","rfn", "cfn","den","avg","dnc"))%>%
      hot_context_menu()
  ...
  # 14. Read HOT table ####
  GetPresepcifiedBlockTypes<-
    observeEvent(input$PreSpecifiedBlocktypesHOT,{
    hot_to_r(input$PreSpecifiedBlocktypesHOT)
  })
  }
